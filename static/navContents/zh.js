export default {
  newNode: [{
    title: '部署节点',
    content: `<div>自动各个主机安装FISCO BCOS 区块链节点 和 区块链BaaS平台-Front 节点前置服务
           <br/><br/>
           <p> 部署节点包含三步操作：</p>
           <p>1.&nbsp添加主机</p>
           ...
           <p>-&nbsp自动加载镜像时，会在节点主机指定目录中创建download目录，并下载镜像的tar包保存于此；若已存在同名tar包将不重复下载，直接使用已有tar包(docker-fisco-区块链BaaS平台.tar)</p>
           <br/>
    ...
    content: `<div>搭链失败或需要重新建链时，可以删除链后重新搭建。
            <br/>
            <p>详情请参考区块链BaaS平台的可视化部署文档。</p>
            <a target='_blank' href='https://区块链BaaS平台doc.readthedocs.io/zh_CN/latest/docs/区块链BaaS平台-Install/visual_deploy.html'>https://区块链BaaS平台doc.readthedocs.io/zh_CN/latest/docs/区块链BaaS平台-Install/visual_deploy.html</a>       
            `
  }, {
    title: '节点列表',
    ...
    title: '新增节点前置',
    content: `<div>区块链BaaS平台需要添加节点前置后才能对链进行管理
            <br/>
  ...
  host: [{
    ...
    content: `<div>主机管理中的主机是用于可视化部署中安装节点的主机（以下简称节点主机）
                ...
                <br/>
                <p>注意：</p>
                <p>-&nbsp节点主机需要保证区块链BaaS平台所在主机的免密登录</p>
                <p>-&nbsp节点主机需要安装docker、配置docker用户组、配置Ansible用户的sudo权限、安装FISCO BOCS节点依赖等</p>
                <p>-&nbsp若填写的是127.0.0.1的主机，则所有节点将只能安装在区块链BaaS平台服务所在机器上</p>
                <br/>
                <p>详情请参考区块链BaaS平台的可视化部署文档。</p>
                <a target='_blank' href='https://区块链BaaS平台doc.readthedocs.io/zh_CN/latest/docs/区块链BaaS平台-Install/visual_deploy.html'>https://区块链BaaS平台doc.readthedocs.io/zh_CN/latest/docs/区块链BaaS平台-Install/visual_deploy.html</a>s    
                `
  }],
  contractList: [{
      title: '已登记合约列表',
      content: `<div>包含使用区块链BaaS平台管理台部署的合约（不包含在区块链BaaS平台-Front页面部署的合约）
                <br/>
                <p>-&nbsp可以通过“导入ABI”按钮将节点前置或控制台等其他方式部署的合约导入到区块链BaaS平台中进行管理</p>
                <p>-&nbspABI编码功能可以根据合约的ABI与合约方法的入参编码得到交易的input值</p>               
                `
    },
    {
      title: '全量合约列表',
      content: `<div>包含链上部署的所有合约，其中已登记合约会排序在前，未登记的合约（不在区块链BaaS平台中部署的合约）
                <br/>
                <p>-&nbsp可以通过“导入ABI”按钮，填入合约ABI和合约名称后导入到区块链BaaS平台进行管理</p>                          
                `
    }
  ],
  cnsManagement: [{
    title: 'CNS查询',
    content: `<div>可以根据合约名和合约版本查询CNS记录的所有合约，在区块链BaaS平台的“合约IDE”中部署合约时可以勾选“CNS”选项，进行合约部署的同时也注册CNS
                <br/>
                <p>-&nbsp链上CNS可以查询链上CNS注册的合约</p>
                <p>-&nbsp本地CNS会记录在区块链BaaS平台中缓存的CNS数据</p>                                         
                `
  }],
  ...
  eventCheck: [{
    ...
    content: `<div>根据合约地址、合约ABI及合约event在指定的区块范围内过滤得到合约事件列表。
                <br/>
                 <p>-&nbsp选中区块链BaaS平台中已有的合约地址可以自动填入合约ABI，并根据下拉框的event名来进行event过滤；也可以手动填入相应的信息</p>
                 <p>-&nbsp此功能仅用于页面展示event信息，若需要使用event的事件推送功能，则参考区块链BaaS平台-Front的事件订阅功能，详情可在区块链BaaS平台-Front文档-附录中的“链上事件订阅”查看</p>
  ...
  privateKeyManagement: [{
      title: '已登记列表',
      content: `显示在区块链BaaS平台管理台创建的私钥用户，包含私钥用户和公钥用户两种。私钥用户代表拥有私钥信息的用户，私钥托管在密钥管理服务中；公钥用户则只保存了地址信息。
                <br/>
                 <p>-&nbsp新增用户：可以创建一个新的随机的私钥并保存，也可以输入一个地址创建一个公钥用户</p>
                 <p>-&nbsp导入私钥：支持导入十六进制/十进制明文私钥、.pem格式、.p12格式私钥文件，也可以导入数字身份管理平台的十进制.txt格式私钥文件</p>
                 <p>-&nbsp导出私钥：支持将区块链BaaS平台中记录的私钥导出为.p12格式的私钥文件（要求密钥管理配置允许导出私钥</p>
                 <p>-&nbsp绑定私钥：支持为公钥用户绑定一个私钥，其格式与“导入私钥”一致 </p>                                       
                `
    },
    {
      title: '全量列表',
      content: `显示所有链上发交易的私钥地址，包括已登记和未登记的私钥信息。可以通过“导入”按钮将未登记的私钥导入到区块链BaaS平台中。                                  
                `
    }
  ],
  newPermission: [{
  ...
  certificate: [{
    ...
    content: `证书管理：支持导入和查看证书信息，包括查看Front的sdk证书、Front对应节点的链证书、机构证书、节点证书，可查看证书内容、证书有效期、证书链关系等信息；
                ...
                平台将默认加载所有Front的证书，需要在区块链BaaS平台-Front配置文件中配置nodePath节点路径；</p>
  ...
  emailAlarm: [{
    ...
    content: `可配置邮件告警所用到的邮件服务器相关参数，包含邮件协议类型protocol、邮件服务器地址host、服务使用端口port、用户邮箱地址username、用户邮箱授权码password；鉴权选项包含Authentication验证开关authentication（默认开启）：
                ...
                <br/>
                <p>详情请参考区块链BaaS平台的可视化部署文档</p>
                <a target='_blank' href='https://区块链BaaS平台doc.readthedocs.io/zh_CN/latest/docs/区块链BaaS平台-Console-Suit/index.html#mail-use'>https://区块链BaaS平台doc.readthedocs.io/zh_CN/latest/docs/区块链BaaS平台-Console-Suit/index.html#mail-use</a>
       `
  }],
  emailAlarmType: [{
    title: '告警类型配置',
    content: `包含了告警类型的配置（节点状态告警、审计告警、证书有效期告警三种），告警日志的查看；
                ...
                <br/>
                 ...
                 <br/>
                 <br/>
                其中在区块链BaaS平台-Node-Manager的配置文件application.yml的constant可以配置定时任务定时监控节点状态、审计状态、证书有效期的频率，监控到异常状态时将触发邮件告警，发送告警邮件到联系人邮箱，同时按配置的间隔时间定时重复发送告警邮件，直到异常状态消除；
                <br/>
  ...
  unusualUser: [{
    ...
    content: `交易审计是结合区块链数据，私钥管理和合约管理三者的数据，以区块链数据为原材料，以私钥管理和合约管理为依据做的一个综合性的数据分析功能。交易审计提供可视化的去中心化合约部署和交易监控、审计功能，方便识别链资源被滥用的情况，为联盟链治理提供依据。
                <br/><br/>
                异常用户会统计监控链上出现的异常交易用户，即：没在区块链BaaS平台登记的交易用户。
                <br/><br/>
                若异常用户数达到后台配置的最大值，异常过多时(默认大于等于20)，会停止审计。
                <br/>
                此时需要查看交易情况，找出异常原因后，导入异常用户的地址到区块链BaaS平台中即可清理异常记录。                
                `
  }],
  unusualContract: [{
    title: '异常合约',
    content: `交易审计是结合区块链数据，私钥管理和合约管理三者的数据，以区块链数据为原材料，以私钥管理和合约管理为依据做的一个综合性的数据分析功能。交易审计提供可视化的去中心化合约部署和交易监控、审计功能，方便识别链资源被滥用的情况，为联盟链治理提供依据。
                <br/><br/>
                异常合约会统计监控链上合约部署情况，标记非白名单合约为异常用户，即：没在区块链BaaS平台中登记的合约
                <br/><br/>
                若异常合约数达到后台配置的最大值，异常过多时(默认大于等于20)，会停止审计。
                <br/>
                此时需要查看交易情况，找出异常原因后，导入异常合约的地址到区块链BaaS平台中即可清理异常记录。                               
                `
  }],
  ...
  blockEvent: [{
    ...
    content: `订阅事件管理：可查看前置中已订阅的链上事件通知，包括出块事件列表和合约Event事件列表。
                ...
                <a target='_blank' href='https://区块链BaaS平台doc.readthedocs.io/zh_CN/latest/docs/区块链BaaS平台-Front/appendix.html#event_subscribe'>https://区块链BaaS平台doc.readthedocs.io/zh_CN/latest/docs/区块链BaaS平台-Front/appendix.html#event_subscribe</a>
       `
  }],
  // contractEvent: [{
  //   title: '合约事件',
  ...
  accountInfo: [{
    ...
    content: `账号管理提供管理台登陆账号的管理功能。管理台用户分为三种角色：
                <br/>
                 ...
                 <br/>
                开发者模式默认关闭。如需开启此功能，可以在区块链BaaS平台-Node-Manager配置文件application.yml中修改developerModeEnable为true，然后重启服务。
                <br/><br/>
  ...
  groupManagement: [{
    ...
    content: `群组管理提供动态管理群组的功能，包含：生成群组（生成群组配置文件到单个或多个节点）、启动群组、停止群组、移除群组（标记为移除）、恢复群组、删除群组数据等操作
                ...
                <br/>
                 <p>详情使用说明可以区块链BaaS平台使用手册文档中的附录-动态群组管理使用指南</p>
                 <a target='_blank' href='https://区块链BaaS平台doc.readthedocs.io/zh_CN/latest/docs/区块链BaaS平台-Console-Suit/index.html#dynamic_group_use'>https://区块链BaaS平台doc.readthedocs.io/zh_CN/latest/docs/区块链BaaS平台-Console-Suit/index.html#dynamic_group_use</a>
